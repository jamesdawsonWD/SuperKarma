...
import {ISuperKarmaGovernance} from './interfaces/ISuperKarmaGovernance.sol';
import {IGovernanceStrategy} from './interfaces/IGovernanceStrategy.sol';
import {IProposalValidator} from './interfaces/IProposalValidator.sol';
import '@openzeppelin/contracts/math/SafeMath.sol';

/**
 * @title Proposal Validator Contract, inherited by  SuperKarma Governance Executors
...
contract ProposalValidator is IProposalValidator {
    ...
    function validateCreatorOfProposal(
        ISuperKarmaGovernance governance,
        address user,
        uint256 blockNumber
     ...
     **/
    function getMinimumPropositionPowerNeeded(ISuperKarmaGovernance governance, uint256 blockNumber)
        public
        view
        override
        returns (uint256)
    {
        ...
        return
            currentGovernanceStrategy.getTotalPropositionSupplyAt(blockNumber).mul(PROPOSITION_THRESHOLD).div(
     ...
     **/
    function isProposalPassed(ISuperKarmaGovernance governance, uint256 proposalId)
        external
        view
        override
        returns (bool)
    {
        return (isQuorumValid(governance, proposalId) && isVoteDifferentialValid(governance, proposalId));
    }
     ...
     * @return voting power needed for a proposal to pass
     **/
    function isQuorumValid(ISuperKarmaGovernance governance, uint256 proposalId)
        public
        view
        override
        returns (bool)
    {
        ISuperKarmaGovernance.ProposalWithoutVotes memory proposal = governance.getProposalById(proposalId);
     ...
     **/
    function isVoteDifferentialValid(ISuperKarmaGovernance governance, uint256 proposalId)
        public
        view
        override
        returns (bool)
